  ...
  return (
    <div className={classes.page}>
      <Container size="xl" py="xl">
        ...
        >
          <Title order={1} className={classes.title}>{location.name}</Title>
        </motion.div>
        ...
        >
          {photos.length > 0 ? (
            ...
            <Box py="xl" my="xl">
              <motion.div {...ANIMATION.fadeIn} transition={{ duration: ANIMATION.duration.normal }}>
                <Center>
                  <Text c="dark.2" size="lg">No images found for {location.name}</Text>
                </Center>
              </motion.div>
            </Box>
          )}
